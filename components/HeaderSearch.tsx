...
export default function HeaderSearch() {
  ...
  return (
    <form onSubmit={handleSubmit} className="relative flex-1 min-w-0 md:max-w-2xl">
      ...
      />
      {mostrarSugestoes && (
        <ul className="absolute top-full left-0 right-0 mt-1.5 bg-white dark:bg-zinc-900 border border-gray-500 dark:border-zinc-800 rounded-xl shadow-soft-lg overflow-hidden z-50">
          {sugestoes.map(termo => (
            <li key={termo}>
              <button
                type="button"
