    ...
    <button
      ...
      className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
        selecionado
          ...
          : disabled
            ? 'bg-white text-gray-300 border-gray-500 cursor-not-allowed'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          {servicos.map(s => (
            <Link
              key={s.href}
              href={s.href}
              className="block bg-white border border-gray-500 rounded-xl p-4 hover:border-lm-green/40 transition-colors"
            >
